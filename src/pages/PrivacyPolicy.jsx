...
const PrivacyPolicy = () => {
    ...
    useEffect(() => {
        const loadPage = async () => {
            try {
                setLoading(true);
                const response = await fetchStaticPage(SLUGS.PRIVACY);
                if (response?.success) {
    ...
    if (!pageData) {
        return (
            <div className="container mx-auto px-4 py-20 text-center" dir={isRTL ? 'rtl' : 'ltr'}>
                <h1 className="text-2xl font-bold mb-4">{language === 'ar' ? 'سياسة الخصوصية' : 'Privacy Policy'}</h1>
                <p className="text-gray-500 mb-8">{error ? 'Error loading privacy policy.' : 'Privacy policy content is currently being updated. Please check back later.'}</p>
